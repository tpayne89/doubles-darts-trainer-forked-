...
export default function AppTrebles({ options }) {
  const navigate = useNavigate();
  const { endOption = "end", enableSkips = false } = options || {};
  const formatTrebleLabel = (t) => {
  ...
  // Base trebles 1-20
  const trebles = Array.from({ length: 20 }, (_, i) => i + 1);
  ...
  const [skippedTrebles, setSkippedTrebles] = useState([]);
  const leaderboardKey = `leaderboard_trebles_${endOption}`;
  ...
  const logThrow = (result) => {
    ...
    const newThrow = { result, treble: currentTarget };
  ...
  const skipCurrentTarget = () => {
    ...
    setSkippedTrebles((prev) => [...prev, currentTarget]);
  ...
  const submitThrows = () => {
    if (pendingThrows.length === 0) {
      const misses = Array(3).fill({ result: "miss", treble: currentTarget });
    ...
    if (
      hitThisRound &&
      lastHitThrow &&
      lastHitThrow.treble === targets[targets.length - 1]
    ) {
      setShowUsernamePrompt(true);
    }
  };
  ...
  const statsByTarget = targets.map((target) => {
    const throwsForTarget = allThrows.filter(
      (t) => t.treble === target && t.result !== "skip"
    );
    const attempts = throwsForTarget.length;

    if (skippedTrebles.includes(target)) {
      return { treble: target, attempts, rate: "-" };
    }
  ...
  const statsWithCumulative = statsByTarget.map(
    ({ treble, attempts, rate }, i) => {
      if (treble > highestTargetWithAttempts) {
        return { treble, attempts: 0, rate: "-", cumulative: 0 };
      }
      const cumulative = statsByTarget
        .slice(0, i + 1)
        .reduce((sum, item) => sum + item.attempts, 0);
      return { treble, attempts, rate, cumulative };
    }
  );
  ...
  const printResults = () => {
    ...
    doc.setFontSize(18);
    doc.text("Darts Trebles Trainer Results", 14, 20);
    doc.setFontSize(12);
    ...
    headlineStats.push([
      "Trebles Skipped",
      `${skippedTrebles.length} (${skippedTrebles
        .map((d) => formatTrebleLabel(d))
        .join(", ")})`,
    ]);
    ...
    const statsHeaders = [["Treble", "Attempts (Cumulative)", "Hit Rate"]];
    const statsData = statsWithCumulative.map(
      ({ treble, attempts, cumulative, rate }) => [
        formatTrebleLabel(treble),
        `${attempts} (${cumulative})`,
        `${rate}%`,
      ]
    );
    ...
    const logData = submittedRounds.map((round) =>
      round.map((t) =>
        t.result === "hit"
          ? `Hit ${formatTrebleLabel(t.treble)}`
          : `Miss ${formatTrebleLabel(t.treble)}`
      )
    );
  ...
  return (
    <div className="app-container">
      ...
      <h1 className="title">Darts Trebles Trainer</h1>
      ...
      <div className="button-group-container">
        <div className="button-group-trebles">
          <button
          ...
          <button
            ...
            disabled={pendingThrows.length >= 3 || isGameOver}
          >
            {formatTrebleLabel(currentTarget)}
          </button>
          <button
      ...
      <div className="undo-group-container">
        <div className="undo-group-trebles">
          <button
      ...
      {showLeaderboard && (
        <div className="modal-overlay">
          <div className="modal">
            <h2>Leaderboard – Trebles ({getEndOptionLabel(endOption)})</h2>
        ...
        <table className="stats-table">
          ...
          <tbody>
            {statsWithCumulative.map(
              ({ treble, attempts, cumulative, rate }) => (
                <tr key={treble} style={{ color: getHitRateColor(rate) }}>
                  <td>{formatTrebleLabel(treble)}</td>
      ...
      >
        {showHeatmap && (
          <DartboardHeatmap
            stats={statsByTarget.map(({ treble, attempts }) => ({
              double: treble,
              attempts,
            }))}
          />
